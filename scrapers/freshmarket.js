...
const CHAIN = 'freshmarket';
const BASE_URL = 'https://www.freshmarket.co.il/search?q=';
...
async function scrapeProduct(query) {
  ...
  try {
    ...
    console.log(`[freshmarket] "${query}" → ${best.name} ₪${best.price}`);
    return { name: best.name, price: best.price, unit: '' };

  } catch (err) {
    logScrape(CHAIN, 'error', `Error for "${query}": ${err.message}`);
    console.error(`[freshmarket] Error:`, err.message);
    return null;
  } finally {
