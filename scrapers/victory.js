...
const CHAIN = 'victory';
const BASE_URL = 'https://www.victoryonline.co.il/search?q=';
...
async function scrapeProduct(query) {
  ...
  try {
    ...
    // המתן לרינדור Angular
    await page.waitForSelector('.carousel-component-product-item', { timeout: 10000 }).catch(() => {});
    ...
    const products = await page.evaluate((q) => {
      ...
      containers.forEach(el => {
        ...
        // מחיר מוחלט (לא מחיר ל-100 גרם/מ"ל)
        let price = null;
        el.querySelectorAll('.price').forEach(priceEl => {
          const text = priceEl.textContent.trim();
          if (!text.includes('ל-') && text.includes('₪')) {
            const num = parseFloat(text.replace(/[^\d.]/g, ''));
            if (!isNaN(num) && num > 0 && (price === null || num < price)) price = num;
          }
        });
